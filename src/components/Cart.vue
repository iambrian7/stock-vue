<template>
<div class="cart">
    <modal v-if="active" @close="active = false" >
    <!--
      you can use custom content here to overwrite
      default content
    -->
    <h3 slot="header">custom header</h3>
    <div class="cart-body" slot="body">
    <div class="modal-body step-1" data-step="1"  v-if="state == 1">
        <div id="cart-items">
           
            <table v-if='purchasedProducts.length > 0' class="table" id="cart-items-table">
                <thead>
                <tr><th>Product</th><th>Quanity</th><th>Price</th><th>Totals</th></tr>
                </thead>
                <tbody>
                 <tr v-for="p in purchasedProducts"><td>{{p.name}}</td><td>{{p.qty}}</td>{{p.price}}<td>{{p.qty * p.price}}</td></tr>               
                </tbody>
            </table>
             <h2 v-else>No items in cart</h2>
            <div class="cart-totals"></div>
        </div>
        </div>
        <div class="modal-body step-2" data-step="2" v-if="state == 2">
            <div class="joes-hidden-checkout">
                <div class="items-summary">
                    <h3> <div class="cart-totals-summary"></div></h3>
                </div>
                <div class="card-row">
                    <span class="visa"></span>
                    <span class="mastercard"></span>
                    <span class="amex"></span>
                    <span class="discover"></span>
                </div>
                <!--<form class="form-inline" name="cardform" id="cardform">-->
                <div id="result" class='alert alert-danger'></div>
                <div class="form-group">
                    <label class="sr-only" for="card_name">Name</label>
                    <input type="text" class="form-control" id="card_name" placeholder="Name" required="required" >
                </div>
                <div class="form-group">
                    <label class="sr-only" for="card_card">Card Number</label>
                    <input type="text" class="form-control" id="card_card" placeholder="Card Number" required="required" >
                </div>
                <div class="form-group">
                    <label class="sr-only" for="card_date">Expiration month/date</label>
                    <input type="text" class="form-control" id="card_date" placeholder="Expires (mm/yy)" required="required" >
                </div>    
                <div class="form-group">
                    <label class="sr-only" for="card_cvc">CVC</label>
                    <input type="text" class="form-control" id="card_cvc" placeholder="CVC" required="required" >
                </div>
                <div class="form-group">
                    <label class="sr-only" for="card_zip">Zip code</label>
                    <input type="text" class="form-control" id="card_zip" placeholder="Zip code" required="required" >
                </div>
            </div>
        </div>
        <div class="modal-body step-3" data-step="3" v-if="state == 3">
            <h1> Complete. Thank You</h1>
            <div id="downloads">
                <div class="download-item">
                    <h3>Your purchases can be downloaded here:  </h3>
                <!-- <button class="btn btn-success purchased-item"> <a href="./downloads/Busplan.docx">Business Plan</a></button>
                    <button class="btn btn-success purchased-item"> <a href="./downloads/Busplan.docx">Business Plan</a></button> -->
                </div>
            </div>
        </div>

    </div>
    <div class="modal-footer" slot="footer">
        <button type="button" class="btn btn-default" data-dismiss="modal" @click="$emit('close')">Close</button>
        <button type="button" class="btn btn-primary step step-1" data-step="1"  v-if="state==1"@click="state=2">Checkout</button>
        <button type="button" class="btn btn-primary step step-2" data-step="2" v-if="state==2" @click="state=1">Edit Cart</button>
        <button type="button" class="btn btn-primary step step-2" data-step="2" v-if="state>1"@click="state=3">Submit Purchase</button>
    </div>
  </modal>
</div>
</template>

<script>
import Modal from '@/components/Modal'
export default {
  name: 'cart',
  props: ['active','myproducts'],
  components: {'modal':Modal},
  data () {
    return {
      msg: 'Cart component',
      cart: {
          items: [],
          itemsObj: {}
      },
      stripeCard: {},
      state: 1
    }
  },
  methods: {
      sendEvent: function(state){

      },
    fetch: function () {
            var cart = JSON.parse(localStorage.getItem('joesCart') || '{}')
            if (!cart.items) cart.items = [];
            if (!cart.itemsObj) cart.itemsObj = {};
            return cart
    },
    save: function (cart) {
        localStorage.setItem('joesCart', JSON.stringify(cart))
    }
  },
  computed: {
      purchasedProducts: function(){
          return this.myproducts.filter(function(p){
              console.log("filtering products " + p.name)
              return p.purchased;
          })
      }
  },
  created: function(){
      console.log("create Cart.vue.......")
  },
  updated: function(){
      console.log("updated Cart.vue")
  }
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style>
 .m-progress-bar {
            min-height: 1em;
            background: #c12d2d;
            width: 5%;
        }
        .modal-title {background: gainsboro; padding: 0; text-align: center;}
        nav { display: flex; justify-content: flex-end;}
        #cart-button {width: 74px;
            height: 32px;
            border: 1px solid black;
            border-radius: 5px;
            background: red;
            margin: 10px;
            text-align: center;
            padding: 5px;
            font-size: 2.0 em;
            cursor: pointer;}
            .card-row {
        text-align: center;
        margin: 20px 25px 10px;
        }

        #card_form li {text-decoration: none; }
        #card_form {display: flex; flex-direction: column;align-items: flex-start;padding-left: 30px;}
        #card_form div {
            display: flex;
            flex: 0 0 30px;
            align-items: center;
        }
        #card_form label {
            display: flex;
            flex: 0 0 100px;
            padding-right: 30px;

        }
        #card_form input,  #card_form text-area {
            flex: 0 0 200px;
        }

        .card-row span {
        width: 48px;
        height: 30px;
        margin-right: 3px;
        background-repeat: no-repeat;
        display: inline-block;
        background-size: contain;
        }
        .amex {
        background: url("data:image/png;base64,") #fff;
        }

        .visa {
        background: url("data:image/png;base64,") #fff;
        }

        .mastercard {
        background: url("data:image/png;base64,") #fff;
        }

        .discover {
        background: url("data:image/png;base64,") #fff;
        }

        .expiry-date-group {
        float: left;
        width: 50%
        }

        .expiry-date-group input {
        width: calc(100% + 1px);
        border-top-right-radius: 0;
        border-bottom-right-radius: 0;
        }

        .expiry-date-group input:focus {
        position: relative;
        z-index: 10;
        }

        .security-code-group {
        float: right;
        width: 50%
        }

        .security-code-group input {
        border-top-left-radius: 0;
        border-bottom-left-radius: 0;
        }

        .zip-code-group {
        clear: both;
        }

        #PayButton {
        outline: 0!important;
        height: 42px;
        font-size: 16px;
        background-color: #54C7C3!important;
        border: none;
        }

        #PayButton:hover {
        background-color: #6DCECB!important;
        }

        #PayButton:active {
        background-color: #4FBCB9!important;
        }

        #PayButton:disabled {
        background: rgba(84, 199, 195, .5)!important;
        color: #FFF!important;
        }
        /* input:focus:required:invalid {
        background: pink url(ico_validation.png) 379px 3px no-repeat;
        } */
        input:required:valid {
        background-color: #fff;
        background-position: 379px -61px;
        }
        .purchased-item { margin: 10px;}
        .purchased-item a {color: white;}
        .test-mode { color: white;}
        .delete-item {cursor: pointer;}
</style>

